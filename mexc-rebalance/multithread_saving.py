...
class MexcWSClient():
    ...
    def __init__(self):
        ...
        self.symbol_deque_map = self._create_deque_map()

        self.last_ping = int(time.time())
        self.save_count = 0 
        self.save_index = 0

    def _create_deque_map(self):
        symbol_deque_map = {}

        for symbol in self.channels:
            symbol_deque_map[symbol] = deque()
        
        # Adds start signal message to queue
        symbol_deque_map["OSMO_USDT"].append({"begining": int(time.time()) })
        return symbol_deque_map
        ...
        try:
            while True:
                for key, message_list in self.symbol_deque_map.items():
                    ...
                    try:
                        self.db[f"{key}_orderbook"].insert_many(message_list, ordered=False)
